...
class MindServer:
    ...
    def init(self, port=8080):
        ...
        node_script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'init-mindserver.js'))
        ...
        try:
            ...
            self.connected = True
            print("Connected to MindServer. MindServer is initialized.")
...
mindserver_instance = MindServer()

def init(port=8080):
    mindserver_instance.init(port)

def create_agent(settings_json):
    mindserver_instance.create_agent(settings_json)
    
def shutdown():
    mindserver_instance.shutdown()

def wait():
    mindserver_instance.wait()
